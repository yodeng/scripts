...
###  change the default encode type to 'utf-8', 
...
def parse_para():
    ...
    parser.add_argument("-m","--modules",type=str,help='the analysis modules you have done. it should be same as you config.txt file, ####### May check carefully when given!!!!!!. ["cleandata","align","cufflinks","cuffnorm","cuffdiff","snp","noveltranscripts","gokegg","coexpression","propro","cluster","asprofile"] can be chosen',choices = ["cleandata","align","cufflinks","cuffnorm","cuffdiff","snp","noveltranscripts","gokegg","coexpression","propro","cluster","asprofile"],required = True,nargs="+",metavar="module")
    ...
    parser.add_argument("-res","--result_dir",type = str, help="the project results directory",required = True)
    parser.add_argument("-ppi","--ppi",type=int,help="the resulation of picture in output docxfile, default: 300",default=300)
    parser.add_argument("-vs","--vs",type=str,help="the diff groups, like CasegroupA-VS-ControlgroupB CasegroupC-VS-ControlgroupD ...", required = True, nargs="+")
...
def change_file_format(suffix,path,type):
    fn = ""
    if type == "pdf":
        for f in listresultdir:
            if f.endswith(suffix):
                fp = f.partition(path)
                fn = os.path.join(logdir,"pdf_png",fp[-1].lstrip("/") + ".png")
                if not os.path.exists(fn):
                    print 'convert ' + f + "  ->  " +fn
                    convert_pdf(f,fn,resolution=args.ppi)
                return fn
        print 'no %s file found in %s, please check the modules set or results dir[%s]'%(suffix,path,path)
        sys.exit(1)
    elif type == "xlsx":
        for f in listresultdir:
            if f.endswith(suffix):
                fp = f.partition(path)
                fn = os.path.join(logdir,"xlsx_txt",fp[-1].lstrip("/") + ".txt")
...
def change_allpdf_format(suffix,path):
    f_list = []
    for f in listresultdir:
        if f.endswith(suffix):                   
            f_list.append(f)
    if len(f_list) == 0:
        print 'no %s file found in %s, please check the modules set or results dir[%s]'%(suffix,path,path)
        sys.exit(1)
    f = sorted(f_list,key=lambda x:os.path.getsize(x),reverse=True)[0]
    fp = f.partition(path)
    fn = os.path.join(logdir,"pdf_png",fp[-1].lstrip("/") + ".png")
    if not os.path.exists(fn):   
        print 'convert ' + f + "  ->  " +fn
...
def get_file_dict(makeflowfile):
    d = {}
    f = os.popen('grep ":" %s'%makeflowfile)
    for line in f:
        i = line.strip().split(":")
        if len(i[0].split())==1 and len(i[1].split()) ==1:
            k = i[0].split()[0]
            d[k] = i[1].split()[0]
    return d

def get_txt(xlsx):
    a = xlsx_txt_dict.get(xlsx_txt_dict.get(xlsx,None),None)
    if a:
        print a
        return a
...
logdir = os.path.join(os.path.dirname(result),"MakeflowLog")
if not os.path.exists(logdir): os.makedirs(logdir)
random_dir = logdir + "/tmp" + os.popen('echo $RANDOM').read().strip()
...
eg_sample = sorted(sn[:min(6,len(sn))],reverse=False)
eg_sample = [re.sub("\W",lambda x:"\\" + x.group(0),i) for i in eg_sample]
...
xlsx_txt_dict = get_file_dict(os.path.join(logdir,"result_makeflow")) if os.path.exists(os.path.join(logdir,"result_makeflow")) else {}
...
if "clean" in module:
    pic1 = get_file_path(listresultdir,"rawdata/" + sn[0] + "/" + "R1/per_base_quality.png")   ## 得到图片文件在results目录中的路径
    pic2 = get_file_path(listresultdir,"rawdata/" + sn[0] + "/" + "R1/per_base_sequence_content.png")
    pic3 = get_file_path(listresultdir,"cleandata/" + sn[0] + "/" + "summary.png")
    ...
    qcfile = change_file_format("clean_data.state.xlsx",result,"xlsx")
    eg_sn = os.popen("cut -f1 %s"%os.path.join(logdir,"xlsx_txt","2.cleandata","clean_data.state.xlsx.txt")).read().split()[1:]
    eg_sample = sorted(eg_sn[:min(6,len(eg_sn))],reverse=False)
    eg_sample = [re.sub("\W",lambda x:"\\" + x.group(0),i) for i in eg_sample]
...
if "align" in module:
    pic1 = get_file_path(listresultdir,"alignment_statistics/" + sn[0] + "/" + "element_distribution.png")
    pic2 = get_file_path(listresultdir,"alignment_statistics/" + sn[0] + "/" + "uniform_distribution.png")
    pic3 = get_file_path(listresultdir,"alignment_statistics/" + sn[0] + "/" + "saturation.png")
    ...
    qcfile = change_file_format("align.state.xlsx",result,"xlsx")
    for s in eg_sample:
        s_txt = os.popen("grep -P '^%s\t' %s"%(s,qcfile)).read().strip().split("\t")
        rows.append(s_txt)
...
if "assemble" in module:
    pic = get_file_path(listresultdir,"assembly.transcript.length_distribution.png")
    #renamepic(media,pic)
    ...
    qcfile = change_file_format("assembly.state.xlsx",result,"xlsx")
...
if "expression" in module:
    ...
    for expfileindex,expfile in enumerate(["gene.expression.xlsx","isoform.expression.xlsx"]):  
        #qcfile = [i for i in listdir(os.path.join(result,"xlsx_txt")) if os.path.basename(i) == expfile][0]
        qcfile = change_file_format(expfile,result,"xlsx") if not get_txt(get_file_path(listresultdir,expfile)) else get_txt(get_file_path(listresultdir,expfile))
    ...
    qcfile = change_file_format("transid.geneid.refid.pos.xlsx",result,"xlsx") if not get_txt(get_file_path(listresultdir,"transid.geneid.refid.pos.xlsx")) else get_txt(get_file_path(listresultdir,"transid.geneid.refid.pos.xlsx"))
...
if "cor" in module:
    ...
    pic = change_file_format("sample.correlation.pdf",result,"pdf")
    ...
    qcfile = change_file_format("sample.correlation.matrix.xlsx",result,"xlsx")
...
if "deg" in module:
    ...
    pic1 = change_file_format("deg.state.pdf",result,"pdf")
    pic2 = get_file_path(listresultdir,vslist[0] + "/" + vslist[0] + ".plot_scatter.png")
    pic3 = get_file_path(listresultdir,vslist[0] + "/" + vslist[0] + ".plot_valcano.png")
...
if "deggo" in module:
    pic1 = change_file_format("deg.go.class.pdf",result,"pdf")
    pic2 = change_file_format("enrichment.GO.p_value.top.pdf",result,"pdf")
    pic3 = change_allpdf_format("enrichment.GO.hierarchy.biological_process.pdf",result)
    pic4 = change_allpdf_format("enrichment.GO.hierarchy.cellular_component.pdf",result)
    pic5 = change_allpdf_format("enrichment.GO.hierarchy.molecular_function.pdf",result)
...
if "degpathway" in module:
    ...
    pic1 = change_file_format("KEGG_PATHWAY.second.class.pdf",result,"pdf")
    ...
    pic2 = change_file_format("enrichment.Pathway.p_value.top.pdf",result,"pdf")
    renamepic2(pic2,os.path.join(media,"image22"))
    #renamepic(media,pic2)
    for f in listresultdir:
        if os.path.basename(f) == "enrichment.Pathway.xlsx":           
            print f + " for Pathway pics."
            url = get_kegg_pic(f,abbr)
            k01100 = get_k01100_pic(f,abbr)
            print url.split("\n")[-1] + " ------> " + os.path.join(logdir,"pdf_png",url.split("\n")[0] + ".png")
            print k01100 + " ------> " + os.path.join(logdir,"pdf_png",abbr+"01100.png")
            if not os.path.isfile(os.path.join(logdir,"pdf_png",url.split("\n")[0] + ".png")):  ### 后续添加所有kegg pathway链接，下载超时则用下一个
                url_w = get_png_url(url.split("\n")[-1])
                urllib.urlretrieve(url_w,os.path.join(logdir,"pdf_png",url.split("\n")[0] + ".png"),Schedule)
            if not os.path.isfile(os.path.join(logdir,"pdf_png",abbr+"01100.png")):
                k01100_w = get_png_url(k01100)
                urllib.urlretrieve(k01100_w,os.path.join(logdir,"pdf_png",abbr+"01100.png"),Schedule)
            renamepic2(os.path.join(logdir,"pdf_png",url.split("\n")[0] + ".png"),os.path.join(media,"image23"))
            renamepic2(os.path.join(logdir,"pdf_png",abbr+"01100.png"),os.path.join(media,"image24"))
...
if "cluster" in module:
    ...
    pic = change_file_format("cluster/cluster.samples_heatmap.pdf",result,"pdf")
...
if "snp" in module:
    ...
    snp_count_file = [xlsx_txt_dict[k] for k in xlsx_txt_dict if xlsx_txt_dict[k].endswith("snp.readcount.txt")]
    if snp_count_file and os.path.exists(snp_count_file[0]):
        print snp_count_file[0]
        qcfile = snp_count_file[0]
    else:
        qcfile = change_file_format("snp.readcount.xlsx",result,"xlsx")
    rows = os.popen("head -4 %s"%qcfile).read().strip().split("\n")
    ...
    qcfile = [i for i in listresultdir if os.path.basename(i) == "snpeff.vcf"][0]
...
if "novel" in module:
    ...
    qcfile = [i for i in listresultdir if os.path.basename(i) == "novel_transcripts.gtf"][0]
    ...
    qcfile = change_file_format("novel_transcripts.state.xlsx",result,"xlsx") if not get_txt(get_file_path(listresultdir,"novel_transcripts.state.xlsx")) else get_txt(get_file_path(listresultdir,"novel_transcripts.state.xlsx"))
    rows=[]
    for s in eg_sample + ["cuffmerge"]:
        s_txt = os.popen("grep -P '^%s\t' %s"%(s,qcfile)).read().strip().split("\t")
        rows.append(s_txt)
...
if "asp" in module:
    ...
    qcfile = change_file_format("asprofile.state.xlsx",result,"xlsx")
    rows=[]
    for s in eg_sample + ["cuffmerge"]:
        s_txt = os.popen("grep -P '^%s\t' %s"%(s,qcfile)).read().strip().split("\t")
        rows.append(s_txt)
...
if "corexp" in module:
    ...
    pic = change_file_format("coexpression/coexpression.pdf",result,"pdf")
    if pic:
        renamepic2(pic,os.path.join(media,"image27"))
    else:
        coexp = os.path.join(logdir,"pdf_png","coexpression.pdf.png")
    ...
    qcfile = change_file_format("Co-Expression.xlsx",result,"xlsx") if not get_txt(get_file_path(listresultdir,"Co-Expression.xlsx")) else get_txt(get_file_path(listresultdir,"Co-Expression.xlsx"))
    rows = os.popen("head -4 %s"%qcfile).read().strip().split("\n")[1:]
    rows = [i.split("\t") for i in rows]
    rows = [i[:2] + [format(float(i[2]),".5f")] + [format(float(i[-1]),".2e")] for i in rows]
...
if "propro" in module:
    ...
    pic = change_file_format(vslist[0] + "/" + vslist[0] + ".pdf",result,"pdf")
    if pic:
        renamepic2(pic,os.path.join(media,"image28"))
    else:
        propic = os.path.join(logdir,"pdf_png","propro.pdf.png")
    ...
    qcfile = change_file_format("deg.pro-pro.xlsx",result,"xlsx") if not get_txt(get_file_path(listresultdir,"deg.pro-pro.xlsx")) else get_txt(get_file_path(listresultdir,"deg.pro-pro.xlsx"))
    rows = os.popen("head -4 %s"%qcfile).read().strip().split("\n")[1:]
