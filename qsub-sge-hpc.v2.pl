...
=head1 Exmple
  ...
  8.work with user defined min cpu cores: if your program need multiple cores and you do not want to interrupted by other jobs, you could set min cpu cores to prevent other jobs use these cores.
  perl qsub-sge.pl --queue all.q --cores 20 ./work.sh
...
my ($Queue, $Interval, $Lines, $Maxjob, $Convert,$Secure,$Reqsub,$Resource,$Job_prefix,$Verbose, $Help,$Check,$Cores);
GetOptions(
	"lines:i"=>\$Lines,
	"maxjob:i"=>\$Maxjob,
	"interval:i"=>\$Interval,
...
{	
	...
	Version: 
	Usage:
	v1:add check functional
	throw one job and sleep interval,2017/8/10
	...
	--resource <str>  set the required resource used in qsub -l option, default vf=0.9G
	--jobprefix <str> set the prefix tag for qsubed jobs, default work
	--check <str>     check the unfinished job and just run these jobs 
	--verbose         output verbose information to screen   
	--cores           set the min cpu cores, defalut 1
	--help            output help information to screen  
...
#####added by fans 2017/8/4
#####add check functional
my @qsub=glob("$work_shell_file.*.qsub");
if(scalar @qsub !=0 && defined $Check)
{
	my $last_time=0;my $qsub_dir;
	foreach my $q (@qsub)
	{
		my $date_string = stat($q)->mtime;
		if($date_string>$last_time)
		{
			$qsub_dir=$q;
	...
	my @o_file=glob("$qsub_dir/*.sh");
	my %all_job;my %finished_job;
	foreach my $o (@o_file)
	{
		my $id=basename($o);$id=(split/\./,$id)[0];
		my @ff=glob ("$o.o*");
		if(scalar @ff !=0)
		{
			my $content= `cat $o.o*`;
	...
	my $unfinished_file;my $unfinished_num=0;
	foreach my $i (keys %all_job)
	{
		next if(exists $finished_job{$i});
		$unfinished_num++;
		$unfinished_file.="$qsub_dir/$i.sh ";
	}
	my $check_file=$work_shell_file.".$$.check";
	open (OUT,">$check_file.log")||die "can't creat file $check_file.log\n";
	print OUT "there are $unfinished_num unfinished jobs in $work_shell_file\n";
	print OUT "$unfinished_file\n";
	close OUT;
	`cat $unfinished_file |sed 's\/; echo This-Work-is-Completed!\/\/g' > $check_file.sh` ;
	$work_shell_file_globle="$check_file.sh";
}
...
#my $job_cmd = "qsub -cwd -S /bin/sh ";  ## -l h_vmem=16G,s_core=8 
	###added by fanshu 2016/11/1 -S /bin/bash
	my $job_cmd = "qsub -cwd -S /bin/bash " ;#-S /bin/sh ";  ## -l h_vmem=16G,s_core=8 
	$job_cmd .= "-q $Queue "  if(defined $Queue); ##set queue
	$job_cmd .= "-pe local $Cores " ;# Add By Xiaoming Xu
	if(defined $Resource)		##set resource,modified by fans 2017/8/10
	{
		my @r=split/,/,$Resource;
		my $rr=join " -l ",@r;
		$job_cmd .= "-l $rr " if(defined $Resource); 
	}
