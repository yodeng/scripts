...
def change_project_num(body_ele,name):
    a = equal_str_Element(body_ele,u"项目数据分析报告")
    a.getparent().getprevious().getchildren()[1].text = str(name)
...
def modified_info(ele,group_info,vslist,species = "Oryza sativa", fa="Oryza_sativa.IRGSP-1.0.31.dna.toplevel.fa",gtf="Oryza_sativa.IRGSP-1.0.31.gtf",egname=None):
    sn = []
    ...
    equal_str_Element(ele,"Oryza_sativa.IRGSP-1.0.31.dna.toplevel.fa").text = fa
    equal_str_Element(ele,"Oryza_sativa.IRGSP-1.0.31.gtf").text = gtf
    equal_str_Element(ele,"Oryza").text = species.split()[0]
    equal_str_Element(ele,"sativa").text = species.split()[1]
...
def group_elemet(ele,module=["clean","align","assemble","expression","cor","deg","deggo","degpathway","function","cluster","snp","novel","asp","corexp","propro"]):
    allmodules = {"clean":u"数据质控","align":u"序列回帖","assemble":u"转录本组装","expression":u"表达量分析","cor":u"样品间相关性分析","deg":u"差异表达基因分析","deggo":u"差异表达基因GO注释和富集","degpathway":u"差异表达基因Pathway注释和富集","function":u"基因功能注释","cluster":u"差异基因表达模式聚类分析","snp":u"cSNP和InDel分析","novel":u"新转录本预测","asp":u"可变剪接分析","corexp":u"差异基因共表达网络分析","propro":u"差异基因蛋白互作网络分析","reference":u"参考文献"}
...
if "expression" in module:
    ...
    if "assemble" not in module:
        ...
        tmp_e = eledict["expression"][tab1_index-2]
        for i in tmp_e.iter():
            if i.text == u"；":
                tmp_ee = i.getparent()
                break
