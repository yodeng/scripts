...
for line in gtf_f:
	if "\tgene\t" in line:
		...
		ref_ent[ensembl_id] = re.findall('old_locus_tag "(.+?)";',a[-1])[0] if re.findall('old_locus_tag "(.+?)";',a[-1]) else "-"
...
ensembol_product = {}
with open(gtf) as gtf_f:
    for line in gtf_f:
        if "\texon\t" in line:
            a = line.split("\t")
            if re.findall('gene_id "(.+?)";',a[-1]) and re.findall('product "(.+?)";',a[-1]):
                ensembol_product[re.findall('gene_id "(.+?)";',a[-1])[0]] = re.findall('product "(.+?)";',a[-1])[0]

#eupt = {}
#with open(uniport) as uniport_f:
#    for line in uniport_f:
#        eupt[line.split("\t")[4].split()[-1]] = line.split("\t")[0]
    

entry = []	
with open(anno,"w") as f:
	f.write("#Ref_id\tEntrez_id\tUniprot_id\tChr\tStart\tEnd\tSymbol\tGene_type\tDescription\n")
	for i in new_line:
		...
		new_line_list[1] = ref_ent[new_line_list[0]]
		#if new_line_list[1] != "-" and new_line_list[2]=="-" and eupt.has_key(new_line_list[1]):
		#    new_line_list[2] = eupt[new_line_list[1]]       ### 根据uniport中的信息添加uniport id
		new_line_list[-1] = ensembol_product[new_line_list[0]] + "\n" if new_line_list[-1] == "-\n" else new_line_list[-1]
